        ...
        >
          <div className="flex items-center gap-2 flex-1">
            ...
            <CommandPrimitive.Input
              ...
              className={`flex-1 bg-transparent outline-none placeholder:text-white/50 text-white font-light uppercase min-w-0 ${jetbrainsMono.className}`}
            />
          </div>
          <span
            className={`hidden md:block text-[10px] opacity-50 border border-white/20 px-1 font-light shrink-0 ${jetbrainsMono.className}`}
          >
            ⌘ K
          </span>
        </div>
            ...
            <div className="bg-[rgba(5,20,50,0.2)] backdrop-blur-md border border-[rgba(125,133,136,0.4)] shadow-2xl max-h-[300px] overflow-hidden flex flex-col">
              <CommandPrimitive.List className="overflow-y-auto overflow-x-hidden p-0 custom-scrollbar flex-1">
                <CommandPrimitive.Empty
                  className={`py-6 text-center text-sm text-white/50 font-light ${jetbrainsMono.className}`}
                >
                  No results found.
                </CommandPrimitive.Empty>
                ...
                {filteredCountries.map((country) => (
                  <CommandPrimitive.Item
                    ...
                    className={`relative flex cursor-pointer select-none items-center px-4 py-3 text-[15px] outline-none aria-selected:bg-[rgba(255,255,255,0.1)] text-white font-light data-[disabled]:opacity-50 transition-colors uppercase ${jetbrainsMono.className}`}
                  >
                    <span className="flex-1 tracking-wide">
                      {country.country}
                    </span>
                    <span
                      className={`text-[10px] text-white/50 font-light ${jetbrainsMono.className}`}
                    >
                      {country.code}
                    </span>
                  </CommandPrimitive.Item>

...
const Admin1Map: React.FC<Admin1MapProps> = ({ 
  ...
  const [tooltip, setTooltip] = useState<{name: string, value: number, breakdown?: any[], x: number, y: number} | null>(null);
  ...
  const legendValues = maxVal > 0 
      ? [maxVal, Math.floor(maxVal / 2), Math.floor(maxVal / 5)].filter(v => v > 0) 
      : [];
  // Deduplicate
  const uniqueLegendValues = Array.from(new Set(legendValues)).sort((a, b) => b - a);
      ...
      {uniqueLegendValues.length > 0 && (
          <div className="absolute bottom-2 right-2 z-[1000] pointer-events-none">
              ...
              <div className="bg-[rgba(20,20,20,0.6)] border border-white/10 p-2">
                  <div className="text-[9px] text-[#ff9a65] uppercase font-mono mb-1 text-center">Events</div>
      ...
      {tooltip && createPortal(
          ...
          >
             <div className="absolute z-10 pointer-events-none bg-black/40 backdrop-blur-md border border-white/10 p-2 rounded shadow-xl text-xs transform -translate-x-1/2 -translate-y-full mt-[-8px]">
                <div className="text-white font-mono text-[11px] leading-tight mb-2 border-b border-white/10 pb-1 flex justify-between gap-4">
                    <span className="font-bold text-[#ff9a65] max-w-[120px] truncate">{tooltip.name}</span>
                    <span className="text-white/70">{tooltip.value.toLocaleString()}</span>
                </div>
                {tooltip.breakdown && (
                    <div className="space-y-1">
                        {tooltip.breakdown.slice(0, 5).map((item, i) => (
                            <div key={i} className="flex justify-between items-center gap-3 text-[10px]">
                                <div className="flex items-center gap-1.5 min-w-0">
                                    <span 
                                        className="w-1.5 h-1.5 rounded-full shrink-0" 
                                        style={{ backgroundColor: item.color }}
                                    />
                                    <span className="text-white/60 truncate max-w-[100px]">{item.type}</span>
                                </div>

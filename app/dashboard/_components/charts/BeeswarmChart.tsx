...
import { CountryEventLocation } from '@/app/api/selectCountryEventLocations';
...
const BeeswarmChart: React.FC<BeeswarmChartProps> = ({ 
    ...
    useEffect(() => {
        ...
        g.append('g')
            ...
            .call(g => g.selectAll(".tick text").attr("fill", "#fff").attr("opacity", 0.5));
        ...
        g.selectAll("circle")
            ...
            .attr("cy", d => Math.max(d.radius, Math.min(innerHeight - d.radius, d.y)))
            .attr("r", d => d.radius)
            .attr("fill", d => {
                        ...
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2 border-b border-white/10 pb-1 mb-1">
                                <span className={`text-[10px] uppercase font-mono px-1.5 py-0.5 rounded-sm ${
                                    d.source === 'UCDP' ? 'bg-purple-500/20 text-purple-300' :
                                    d.source === 'ACLED' ? 'bg-blue-500/20 text-blue-300' :
                            ...
                            {d.sub_event_type && d.sub_event_type !== d.event_type && (
                                <div className="text-white/60 text-[10px] italic">{d.sub_event_type}</div>
                            )}
                            <div className="flex justify-between items-center mt-1">
                                <span className="text-white/40 text-[10px]">Fatalities</span>
                                <span className="text-red-400 font-mono text-xs">{d.fatalities}</span>
                            </div>
                            {d.admin1 && (
                                <div className="text-white/30 text-[9px] uppercase tracking-wide mt-1">{d.admin1}</div>
                            )}
                        </div>
                    )
                });

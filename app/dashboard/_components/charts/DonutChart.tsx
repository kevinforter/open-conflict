...
const DonutChart: React.FC<DonutChartProps> = ({ 
  ...
  useEffect(() => {
    ...
    const radius = Math.min(chartWidth, chartHeight) / 2 * (showLabels ? 0.65 : 1);
    ...
    if (showLabels) {
        ...
        const labelData = pie(data).filter(d => (d.endAngle - d.startAngle) > 0.10);
        ...
        const labels = labelData.map(d => {
            ...
            let lineCount = 1;
            if (d.data.name === "Explosions/Remote violence" || d.data.name === "Violence against civilians") {
                lineCount = 3;
        ...
        const relax = (group: typeof labels) => {
            ...
            for (let i = 0; i < 8; i++) { // More iterations
        ...
        g.selectAll("text")
           ...
           .each(function(l) {
               ...
               lines.forEach((line, i) => {
                   el.append("tspan")
                     .attr("x", 0)
                     .attr("dy", i === 0 ? (lines.length > 2 ? "-0.8em" : lines.length > 1 ? "-0.3em" : "0.3em") : "1.1em")
                     .text(line);
               });
           });
    }
      ...
      {mounted && tooltip.visible && tooltip.data && createPortal(
          ...
          >
              ...
              <div className="flex items-center gap-2">
                  ...
                  ></div>
                  <span className="text-white font-mono">{Number(tooltip.data.value).toLocaleString()}</span>
              </div>
          </div>,
          document.body
      )}

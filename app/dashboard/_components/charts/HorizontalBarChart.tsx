...
const HorizontalBarChart: React.FC<HorizontalBarChartProps> = ({ 
  ...
  const [tooltip, setTooltip] = useState<{ visible: boolean; x: number; y: number; data: any | null; key?: string; value?: number }>({
    visible: false,
    x: 0,
    y: 0,
    data: null
  ...
  useEffect(() => {
    ...
    const chartHeight = (typeof height === 'number' ? height : dimensions.height) - margin.top - margin.bottom;
    ...
    yAxis.selectAll(".tick text")
      ...
      .each(function(d: any) {
          ...
          let tempTspan = text.text(null).append("tspan").attr("x", -(margin.left - 10));
    ...
    g.selectAll(".layer")
      ...
      .attr("d", d => {
          ...
          // Top-Right Corner
          if (isRight) p += `A ${effR} ${effR} 0 0 1 ${xVal + wVal} ${yVal + effR}`;
          
          // Right Line
          p += `L ${xVal + wVal},${yVal + hVal - (isRight ? effR : 0)}`;

          // Bottom-Right Corner
          if (isRight) p += `A ${effR} ${effR} 0 0 1 ${xVal + wVal - effR} ${yVal + hVal}`;

          // Bottom Line
          p += `L ${isLeft ? xVal + effR : xVal},${yVal + hVal}`;

          // Bottom-Left Corner
          if (isLeft) p += `A ${effR} ${effR} 0 0 1 ${xVal} ${yVal + hVal - effR}`;
  ...
  return (
    <div ref={containerRef} className={`w-full relative ${className}`} style={{ height }}>
      ...
      {mounted && tooltip.visible && tooltip.data && createPortal(
          <div 
          ...
          >
              <div className="font-mono text-white/40 mb-1 border-b border-white/5 pb-1 tracking-wider">
                  {tooltip.data[labelKey]}
              </div>
              <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: colors[tooltip.key!] }}></div>
                  <span className="text-white/70 font-mono text-[10px] uppercase">{tooltip.key}:</span>
                  <span className="text-white font-mono">{Number(tooltip.value).toLocaleString()}</span>
              </div>
          </div>,
          document.body
      )}

...
const SpiderChart: React.FC<SpiderChartProps> = ({
  ...
  useEffect(() => {
    ...
    axis
      ...
      .attr(
        "x2",
        (d, i) => rScale(maxValue) * Math.cos(angleSlice * i - Math.PI / 2),
      )
      .attr(
        "y2",
        (d, i) => rScale(maxValue) * Math.sin(angleSlice * i - Math.PI / 2),
      )
      .attr("class", "line")
    ...
    axis
      .append("text")
      .attr("class", `legend ${geistMono.className}`)
      ...
      .each(function (d, i) {
        ...
        // Calculate position
        const x =
          rScale(maxValue * 1.15) * Math.cos(angleSlice * i - Math.PI / 2);
        const y =
          rScale(maxValue * 1.15) * Math.sin(angleSlice * i - Math.PI / 2);
        ...
        lines.forEach((line, j) => {
          el.append("tspan")
            ...
            .attr(
              "dy",
              j === 0 ? (lines.length > 1 ? "-0.2em" : "0.3em") : "1.0em",
            )
            .style("text-anchor", () => {
              const angle = angleSlice * i;
              if (angle === 0 || Math.abs(angle - Math.PI) < 0.1)
                return "middle";
              if (angle > 0 && angle < Math.PI) return "start";
              return "end";
            })
    ...
    // Draw the Radar Path
    const radarLine = d3
      .lineRadial<{ name: string; value: number }>()
  ...
  return (
    ...
    >
      ...
      {tooltip && (
        <div
          className={`absolute z-50 pointer-events-none px-2 py-1 bg-black/40 border border-white/10 text-white text-xs ${geistMono.className} rounded backdrop-blur-md transform -translate-x-1/2 -translate-y-full`}
          style={{
            left: tooltip.x,
            top: tooltip.y,
          }}
